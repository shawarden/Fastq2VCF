...
contigMerBlocks=$(find ${SAMPLE_PATH} -type f -iwholename "*/split/*/${CONTIG}.bam"; find ${SAMPLE_PATH} -type f -iwholename "*/split/*/${CONTIG}_*.bam"; find ${SAMPLE_PATH} -type f -iwholename "*/split/*/${CONTIG}\**.bam" | tr '\n' ' ')
...
SECONDS=0
HEADER="MD"
CMD="srun -J ${IDN}_MergeMarkDup_${CONTIG}_$SLURM_ARRAY_TASK_ID $(which java) ${JAVA_ARGS} -jar $EBROOTPICARD/picard.jar MarkDuplicates ${PIC_ARGS} ${MARK_ARGS} ${mergeList} OUTPUT=${OUTPUT}" 
